        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="p-3 sm:p-6 space-y-4 sm:space-y-5 max-w-5xl" dir="rtl">
                ...
                {messages.data.length === 0 ? (
                    <div className="bg-white rounded-xl border border-gray-200 flex flex-col items-center gap-3 py-12 sm:py-16 text-center text-gray-400 px-4">
                        ...
                        <div className="md:hidden space-y-3">
                            {messages.data.map((msg) => (
                                <div
                                    ...
                                    className={`bg-white rounded-xl border border-gray-200 p-3 cursor-pointer transition-colors ${!msg.is_read ? 'bg-emerald-50/40 border-emerald-100' : ''}`}
                                >
                                    <div className="flex items-start gap-3">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className="flex items-start justify-between gap-2">
                                                <p className={`${!msg.is_read ? 'font-bold' : 'font-medium'} text-gray-900 text-sm truncate`}>{msg.name}</p>
                                                <span className="text-[11px] text-gray-400 whitespace-nowrap shrink-0">{formatDate(msg.created_at)}</span>
                                            </div>
                                            <p className="text-xs text-gray-400 truncate" dir="ltr">{msg.email}</p>
                                            <p className={`${!msg.is_read ? 'font-semibold' : ''} text-gray-700 text-sm mt-1.5 line-clamp-1`}>{msg.subject}</p>
                                            <p className="text-xs text-gray-400 mt-0.5 line-clamp-2">{msg.message}</p>
                                        </div>
                                    </div>
                                    <div className="flex justify-end gap-1 border-t border-gray-100 pt-2 mt-3 -mb-1">
                                        <button
                                            onClick={(e) => toggleRead(msg, e)}
                                            title={msg.is_read ? 'علامت‌گذاری به‌عنوان خوانده‌نشده' : 'علامت‌گذاری به‌عنوان خوانده‌شده'}
                                            className="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-gray-100 text-gray-500 hover:text-emerald-600 transition-colors"
                                        >
                            ...
                            <table className="w-full text-sm">
                                <thead>
                                    <tr className="border-b border-gray-100 bg-gray-50 text-right">
                                        ...
                                        <th className="px-4 py-3 font-semibold text-gray-600 hidden lg:table-cell">تاریخ</th>
